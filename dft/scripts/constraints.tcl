...
create_clock -name $REF_CLK_NAME -period $REF_CLK_PERIOD \
-waveform "[expr $REF_CLK_PERIOD / 2] $REF_CLK_PERIOD" [get_ports reference_clk]
...
if {$SE == 0} {
    ...
    create_generated_clock -master_clock $REF_CLK_NAME -source [get_ports reference_clk] \
    -name $ALU_CLK_NAME -divide_by 1 [get_ports clock_gating_cell_inst/gated_clock]
...
set_input_delay $UART_clk_input_delay -clock $UART_CLK_NAME [get_port ser_data_in]
...
set_output_delay $UART_transmitter_clk_output_delay -clock $UART_TRANSMITTER_CLK_NAME [get_port ser_data_out]
...
# Output load
set_load 0.5 [get_port ser_data_out]
...
set_dont_touch_network {ref_clk clock_gating_cell_inst/gated_clock uart_clk clock_divider_inst/output_clk scan_clk}
